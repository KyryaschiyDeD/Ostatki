...
namespace Остатки.Pages.SettingPages.AddWhiteOrBlackStore
{
	...
	/// </summary>
	public sealed partial class AddWhiteOrBlackStoreLeonardo : Page
	{
		public AddWhiteOrBlackStoreLeonardo()
		{
		...
		{
			ShopList = new ObservableCollection<ShopWhiteOrBlack>(ShopWhiteOrBlackJob.GetShopListSpecifically("Леонардо").OrderBy(x => x.Code));
		...
		{
			ShopWhiteOrBlackJob.CreateNewShop(ShopName.Text, ShopCode.Text, (bool)WhiteRadioButton.IsChecked, (bool)ShopIsOnlyThisCheckBox.IsChecked, "Леонардо");
			Zeroing();
			GoToShopList();
			this.InitializeComponent();
		}
